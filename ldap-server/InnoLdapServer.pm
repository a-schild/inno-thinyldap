...
my $gigasetInternationalPrefix =
  "000";    # Replace + sign in front of numbers with these numbers
my $gigasetRemoveUmlauts =
  0;        # Remove common umlauts from results to fix gigaset ldap problems
...
my $defaultSizeLimit =
  40;   # Max number of results to return when the ldap request has no limit set
my $forceSizeLimit =
  0;    # When set to > 0 then we never return more than this number of results
...
sub search {
    ...
    if ( $forceSizeLimit > 0 ) {
        $sizeLimit =
          $forceSizeLimit < $sizeLimit ? $forceSizeLimit : $sizeLimit;
    }
    ...
    if ( $reqData->{'scope'} ) {
        ...
        if ( !$self->isSpeedDial($base) ) {
            ...
            if ( exists $myFilter->{'and'} ) {
                logdebug("Has and in query");
                for my $and ( @{ $myFilter->{'and'} } ) {
                    logdebug("AND1 query loop");
                    if ( exists $and->{'or'} ) {
                        for my $or ( @{ $and->{'or'} } ) {
                            for my $myORCondition ($or) {
                                logdebug( "Found or condition: "
                                      . Dumper($myORCondition) );
                                my $mySubstrings =
                                  $myORCondition->{'substrings'};
                                my $myEquality =
                                  $myORCondition->{'equalityMatch'};

                                if ( defined($myEquality) ) {
                                    if ( $myEquality->{'attributeDesc'} eq
                                        "telephoneNumber"
                                        || $myEquality->{'attributeDesc'} eq
                                        "mobile"
                                        || $myEquality->{'attributeDesc'} eq
                                        "homePhone"
                                        || $myEquality->{'attributeDesc'} eq
                                    ...
                                    {
                                        my $qNumber =
                                          $myEquality->{'assertionValue'};
                                        $searchExpression = $qNumber;
                                        my $result =
                                          $self->lookupNumber( $qNumber,
                                            $userName, $base );
                                        if ( not defined($result) ) {
                                            logdebug("No match found $qNumber");
                                        }
                                        else {
                                            logdebug( "answer found "
                                                  . Dumper($result) );
                                            push @entries, $result;
                                            $entryFound = 1;
                                            last;
                                    ...
                                    my $qNumber =
                                      $self->parseSubstring($mySubstrings);
                                    $searchExpression = $qNumber;
                                    @entries =
                                    ...
                                    if ( scalar(@entries) == 0 ) {
                                        logdebug(
                                            "No match found $searchExpression");
                                    }
                                    else {
                                        logdebug( "answer found "
                                              . Dumper(@entries) );
                            ...
                            @{ $mySubstrings->{'substrings'} } )
                        {
                                ...
                                || $type eq 'metaSearchText' )
                            {
                                ...
                                my $result =
                                  $self->lookupNumber( $searchExpression,
                                    $userName, $base );
                                if ( not defined($result) ) {
                                    logdebug(
                                        "No match found $searchExpression");
                ...
                logdebug("Case no primary AND");
                logdebug( "Found condition: " . Dumper($myFilter) );

                my $mySubstrings = $myFilter->{'substrings'};
                ...
                if ( defined($myEquality) ) {
                        ...
                        "metaSearchNumber" )
                    {
                        ...
                        loginfo("Not query for equality in NO AND");

                        if (
                            $myEquality->{'attributeDesc'} eq "metaSearchText" )
                        {
                            my $qText = $myEquality->{'assertionValue'};
                            $searchExpression = $qText;
                            @entries =
                              $self->lookupNames( $searchExpression, $userName,
                                $base, $sizeLimit );
                            if ( scalar(@entries) == 0 ) {
                    ...
                    $searchExpression = $qNumber;
                    @entries =
                      $self->lookupNames( $searchExpression, $userName, $base,
                        $sizeLimit );
                    if ( scalar(@entries) == 0 ) {
                        logdebug("No match found $searchExpression");
                    }
                    ...
                    logwarn( "Unhandled AND condition Part NO AND : "
                          . Dumper($myFilter) );
                }
            }
        }
                    ...
                    if ( defined($myEquality) ) {
                        ...
                        if (
                            !(
                                ...
                                || $type eq 'mobile'
                                || $type eq 'homePhone'
                                || $type eq 'metaSearchNumber'
                            )
                          )
                        {
                            logtrace("Name0 search for $qNumber");
                            @entries =
                              $self->lookupNames( $qNumber, $userName, $base,
                                $sizeLimit );
                            if ( scalar(@entries) == 0 ) {
                                logtrace("No match found");
                            }
                        ...
                        {
                            @entries =
                              $self->lookupNames( "", $userName, $base,
                                $sizeLimit );
                            if ( scalar(@entries) == 0 ) {
                                logtrace("No match found");
                            }
        ...
        if ( $self->isSpeedDial($base) ) {
            ...
            if ( defined($myEquality) ) {
                if ( $myEquality->{'attributeDesc'} eq "cn" ) {
                    my $qNumber = $self->{_speed_dial_prefix}
                      . $myEquality->{'assertionValue'};
                    $searchExpression = $qNumber;
                    my $result =
                      $self->lookupNumber( $qNumber, $userName, $base );
        ...
        if ( $entryFound >= 1 ) {
            if ( defined($searchExpression) ) {
                logdebug( "Found result for $searchExpression : "
                      . Dumper(@entries) );
            }
            else {
...
sub queryTelSearch() {
    ...
    try {
        ...
        foreach my $entry ( $entries->get_nodelist ) {
            ...
            my $cn =
              trim( $company . ' ' . $firstname ) . ', ' . $zip . ' ' . $city;
...
sub queryMySQLNumber() {
    ...
    my $dbh =
      DBI->connect( $dsn, $userid, $password, { mysql_enable_utf8 => 1 } )
      or die $DBI::errstr;
    ...
    if ( !$self->isSpeedDial($base) ) {
        ...
        $sth->execute( $qNumber, $qNumber, $qNumber, $qNumber, $qNumber,
            $qNumber )
          or die $DBI::errstr;
        logtrace( "Number of rows found :" . $sth->rows );
        while ( my @row = $sth->fetchrow_array() ) {
            my (
                $addressid,        $company,        $firstname,
                $lastname,         $address,        $zip,
                $city,             $country,        $phone,
                $mobile,           $home,           $speeddial_phone,
                $speeddial_mobile, $speeddial_home, $fax,
                $email
            ) = @row;
            ...
            my $cn =
              $prefix
              . $self->makeDisplayName( $base, $company, $firstname,
                $lastname );
            logdebug( "dn: " . $myDN );
            logdebug( "cn: " . $cn );
            $foundEntry->dn($myDN);
            $foundEntry->add( cn => $cn );
            $self->addResultProperties(
                $foundEntry,     $base,            $finalNr,
                $company,        $firstname,       $lastname,
                $address,        $zip,             $city,
                $country,        $phone,           $mobile,
                $home,           $speeddial_phone, $speeddial_mobile,
                $speeddial_home, $fax,             $email
            );
            $retVal = $foundEntry;
        }
        $sth->finish();
        ...
        if ( $sth1->rows > 0 ) {
            while ( my @row = $sth1->fetchrow_array() ) {
                ...
                $foundEntry->add(
                    ...
                    #                                dn => $dnPart,
                    cn => $self->makeDisplayName(
                        $base, $company, $firstname, $lastname
                    ),
                );
                $self->addResultProperties(
                    $foundEntry,     $base,            $phone,
                    $company,        $firstname,       $lastname,
                    $address,        $zip,             $city,
                    $country,        $phone,           $mobile,
                    $home,           $speeddial_phone, $speeddial_mobile,
            ...
            if ( $sth2->rows > 0 ) {
                while ( my @row = $sth2->fetchrow_array() ) {
                    my (
                        ...
                        $city,             $country,
                        $phone,            $mobile,
                        $home,             $speeddial_phone,
                        $speeddial_mobile, $speeddial_home,
                        $fax,              $email
                    ) = @row;
...
#logdebug("Searchresult SpeeddialMobil: id= $addressid, mobile= $mobile company= $company firstname= $firstname email= $email");
                        ...
                        my $dnPart =
                          "cn=mysql_sdp" . escape_dn_value($addressid);
                        my $myDN       = $dnPart . "," . $base;
                        my $foundEntry = Net::LDAP::Entry->new;
                        logtrace( "dn: " . $myDN );
                        $foundEntry->dn($myDN);
...
sub queryMySQLNames() {
    ...
    $sth->execute( $searchExpression, $searchExpression, $searchExpression )
      or die $DBI::errstr;
    logtrace( "Number of rows found :" . $sth->rows );
    while ( my @row = $sth->fetchrow_array() ) {
        my (
        ...
        my $foundEntry = Net::LDAP::Entry->new;
        my $cn =
          $self->makeDisplayName( $base, $company, $firstname, $lastname );
        logdebug( "dn: " . $myDN );
        logdebug( "cn: " . $cn );
        $foundEntry->dn($myDN);
        $foundEntry->add(
...
sub addResultProperties() {
    ...
    if ( $self->isGigaset($base) ) {
        if ( defined($company) && length($company) > 0 ) {
            $foundEntry->add( company => $self->removeUmlauts($company) );
        }
        if ( defined($firstname) && length($firstname) > 0 ) {
            $foundEntry->add( sn => $self->removeUmlauts($firstname) );
        }
        if ( defined($lastname) && length($lastname) > 0 ) {
            $foundEntry->add( givenName => $self->removeUmlauts($lastname) );
        }
        if ( defined($address) && length($address) > 0 ) {
            $foundEntry->add( postalAddress => $self->removeUmlauts($address) );
        }
        if ( defined($zip) && length($zip) > 0 ) {
            $foundEntry->add( postalCode => $self->removeUmlauts($zip) );
        }
        if ( defined($city) && length($city) > 0 ) {
            $foundEntry->add( l => $self->removeUmlauts($city) );
        }
        if ( defined($country) && length($country) > 0 ) {
            $foundEntry->add( countryCode => $self->removeUmlauts($country) );
        }
        if ( defined($email) && length($email) > 0 ) {
            $foundEntry->add( mail => $self->removeUmlauts($email) );
        }
    }
    else {
        if ( defined($company) && length($company) > 0 ) {
            $foundEntry->add( company => $company );
        }
        if ( defined($firstname) && length($firstname) > 0 ) {
            $foundEntry->add( sn => $firstname );
        }
        if ( defined($lastname) && length($lastname) > 0 ) {
            $foundEntry->add( givenName => $lastname );
        }
        if ( defined($address) && length($address) > 0 ) {
            $foundEntry->add( postalAddress => $address );
        }
        if ( defined($zip) && length($zip) > 0 ) {
            $foundEntry->add( postalCode => $zip );
        }
        if ( defined($city) && length($city) > 0 ) {
            $foundEntry->add( l => $city );
        }
        if ( defined($country) && length($country) > 0 ) {
            $foundEntry->add( countryCode => $country );
        }
        if ( defined($email) && length($email) > 0 ) {
            $foundEntry->add( mail => $email );
        }
    }
    if ( defined($phone) && length($phone) > 0 ) {
        $foundEntry->add(
            telephoneNumber => $self->makeResultNumber( $base, $phone ) );
    }
    if ( defined($mobile) && length($mobile) > 0 ) {
        $foundEntry->add( mobile => $self->makeResultNumber( $base, $mobile ) );
    }
    if ( defined($home) && length($home) > 0 ) {
        $foundEntry->add(
            homePhone => $self->makeResultNumber( $base, $home ) );
    }
    if ( defined($fax) && length($fax) > 0 ) {
        $foundEntry->add(
            facsimileTelephoneNumber => $self->makeResultNumber( $base, $fax )
        );
    }
    if ( defined($speeddial_phone) && length($speeddial_phone) > 0 ) {
        $foundEntry->add( speedDial => $speeddial_phone );
    }
    if ( defined($speeddial_mobile) && length($speeddial_mobile) > 0 ) {
        $foundEntry->add( speedDialMobile => $speeddial_mobile );
    }
    if ( defined($speeddial_home) && length($speeddial_home) > 0 ) {
        $foundEntry->add( speedDialHome => $speeddial_home );
    }
}
